...
... # Warning: 4 clusters can result in ram problems, sometimes 8 Gb is not enough
```
...
```python
>>> numbbeta = 150
>>> steps_minimizer = 1000
>>> gamma = 0.5 #steepest descent parameter
>>> steps_final = 800000
>>> d = 2.0
...
...     s = s_row[j]
...     beta = 0.55 #initial guess for beta
...     beta_old = beta
...     dbeta = 1
...     i = 0
...
...         Energy, lnpsi = simulate_hydrogen_molecule(s_row[j], beta, steps_minimizer, N)
...
...         i += 1
...
...     #give necessary parameters to the engines. The distributes the walkers over the cores
...     view.push(dict(beta = beta, s = s, d = d, N = int(N / len(c.ids)), steps_final = steps_final), targets = c.ids)
...
...
...     beta = beta_row[j]
...     #give necessary parameters to the engines. This divides the walkers over the cores
...     view.push(dict(beta = beta, s = s, d = d, N = int(N / len(c.ids)), steps = steps), targets = c.ids)
...
...     #print("End result: beta = ",beta," in ", i,"iterations.")
...
...     #Parallel computing the energy
